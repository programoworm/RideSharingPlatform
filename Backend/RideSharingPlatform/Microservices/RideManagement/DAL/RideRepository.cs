...
namespace RideSharingPlatform.Microservices.RideManagement.DAL
{
    public class RideRepository : IRideRepository
    {
        ...
        public int SheduleRide(RideDTO ride)
        {
            ...
            RideSchedule rideSchedule = new()
            {
                RideFare = this.GetFare(ride.VehicleRegistrationNo) * d.DistanceInKMS,
                MotoristUserId = this.Getmotoristid(ride.VehicleRegistrationNo)
            };
            
            int r = this.AddRide(rideSchedule);
            return r;
        }
    }
}
